...
class Rocked_Skills extends WP_Widget {

    function rocked_skills() {
		$widget_ops = array('classname' => 'rocked_skills_widget', 'description' => __( 'Show your visitors some of your skills.', 'rocked') );
        parent::__construct(false, $name = __('Rocked FP: Skills', 'rocked'), $widget_ops);
		$this->alt_option_name = 'rocked_skills_widget';
	...
	<p><em><?php _e('Skill values should be between 0-100 and will be displayed as percentages on the front end.', 'rocked'); ?></em></p>
	...
	<h4><?php _e('Skill 1', 'rocked'); ?></h4>
	<p>
	<label for="<?php echo $this->get_field_id('skill_one'); ?>"><?php _e('First skill name', 'rocked'); ?></label>
	...
	<p>
	<label for="<?php echo $this->get_field_id('skill_one_max'); ?>"><?php _e('First skill value', 'rocked'); ?></label>
	...
	<h4><?php _e('Skill 2', 'rocked'); ?></h4>
	<p>
	<label for="<?php echo $this->get_field_id('skill_two'); ?>"><?php _e('Second skill name', 'rocked'); ?></label>
	...
	<p>
	<label for="<?php echo $this->get_field_id('skill_two_max'); ?>"><?php _e('Second skill value', 'rocked'); ?></label>
	...
	<h4><?php _e('Skill 3', 'rocked'); ?></h4>
	<p>
	<label for="<?php echo $this->get_field_id('skill_three'); ?>"><?php _e('Third skill name', 'rocked'); ?></label>
	...
	<p>
	<label for="<?php echo $this->get_field_id('skill_three_max'); ?>"><?php _e('Third skill value', 'rocked'); ?></label>
	...
	<h4><?php _e('Skill 4', 'rocked'); ?></h4>
	<p>
	<label for="<?php echo $this->get_field_id('skill_four'); ?>"><?php _e('Fourth skill name', 'rocked'); ?></label>
	...
	<p>
	<label for="<?php echo $this->get_field_id('skill_four_max'); ?>"><?php _e('Fourth skill value', 'rocked'); ?></label>
	...
	function update($new_instance, $old_instance) {
		...
		if ( isset($alloptions['rocked_skills']) )
			delete_option('rocked_skills');		  
		  
		return $instance;
	}
	
	function flush_widget_cache() {
		wp_cache_delete('rocked_skills', 'widget');
	}
	...
	function widget($args, $instance) {
		$cache = array();
		if ( ! $this->is_preview() ) {
			$cache = wp_cache_get( 'rocked_skills', 'widget' );
		}
...
?>
		...
		<?php if ($skill_one !='') : ?>
		<div class="roll-progress gradient">
 			<div class="name"><?php echo esc_html($skill_one); ?></div>
			<div class="perc"><?php echo absint($skill_one_max) . '%'; ?></div>
            <div class="bar" data-percent="<?php echo absint($skill_one_max); ?>" data-waypoint-active="yes">
		...
		<div class="roll-progress gradient">
            ...
            <div class="bar" data-percent="<?php echo absint($skill_two_max); ?>" data-waypoint-active="yes">
				<div class="animate"></div>
			</div>
		</div>
		<?php endif; ?> 
		...
		<div class="roll-progress gradient">
            ...
            <div class="bar" data-percent="<?php echo absint($skill_three_max); ?>" data-waypoint-active="yes">
				<div class="animate"></div>
			</div>
		</div>
		<?php endif; ?> 
		...
		<div class="roll-progress gradient">
            ...
            <div class="bar" data-percent="<?php echo absint($skill_four_max); ?>" data-waypoint-active="yes">
				<div class="animate"></div>
			</div>
		</div>
		<?php endif; ?> 
	...
	<?php
		...
		if ( ! $this->is_preview() ) {
			...
			wp_cache_set( 'rocked_skills', $cache, 'widget' );
		} else {
			ob_end_flush();
		}
	}
